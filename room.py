...
class Room():
    ...
    def create_widgets(self):
        self.table = ttk.Treeview(self.frame, columns=("room_no", "type", "status", "staff", "staying"), show="headings")
        ...
        b2=tk.Button(self.frame1,text='Allot Staff',width=10,command=self.allot)
        b3=tk.Button(self.frame1,text='Toggle Status',width=10,command=self.alter)
        b4=tk.Button(self.frame1,text='Refresh',width=10,command=self.refresh)
    ...
    def allot(self):
        ...
        if(len(selectedItems)==0):
            label=tk.Label(self.frame,text="Please select room(s) to allot staff to",fg="red")
            label.pack()
            return
        root=tk.Tk()
        ...
        for item in selectedItems:
            executeProc(staffid,self.table.item(item,"values")[0],procName="set_staff")
    ...
    def alter(self):
        selectedItems=self.table.selection()
        if(len(selectedItems)==0):
            label=tk.Label(self.frame,text="Please select room(s) to toggle",fg="red")
            label.pack()
            return
        for item in selectedItems:
            executeProc(self.table.item(item,"values")[0],procName="toggle_status")
        clear_frame(self.frame)
        clear_frame(self.frame1)
        Room(self.frame,self.frame1)
