...
class RoomSelectionForm(tk.Frame):
    ...
    def create_widgets(self):
        self.room_treeview = ttk.Treeview(self, columns=("room_no", "room_type", "price"), selectmode="extended")
        ...
        for i, room in enumerate(room_data):
            self.room_treeview.insert("", "end", text=str(i+1), values=(room[0], room[1], room[2]))
        ...
        self.recommended_names_label = tk.Label(self, text="Recommended names:")
        ...
        self.check_in_date=DateEntry(self, width=12, background='darkblue',foreground='white', borderwidth=2)
        
        self.check_out_label = tk.Label(self, text="Check-out date:")
        self.check_out_date=DateEntry(self, width=12, background='darkblue',foreground='white', borderwidth=2)
        ...
        self.select_button = tk.Button(self.button_frame, text="Book", command=self.select_rooms)
        ...
        self.recommended_names_listbox.bind("<<ListboxSelect>>", self.select_recommended_name)
    ...
    def recommend_names(self, event):
        ...
        if search_term:
            recommended_names = [name for name, id in self.get_customer_names() if search_term.lower() in name.lower()]
    ...
    def select_recommended_name(self, event):
        ...
        self.customer_name_entry.insert(0, self.recommended_names_listbox.get(self.recommended_names_listbox.curselection()))   
